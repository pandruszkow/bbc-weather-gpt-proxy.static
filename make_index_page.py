...
def generate_index(output_dir: Path) -> str:
    ...
    # Only show by-location (name-based) paths for cleaner UI
    ...
    if not location_dirs:
        ...
        for loc_dir in location_dirs:
            ...
            # Location name is the directory name
            location_name = loc_dir.name
            
            lines.append(f"  <div class=\"location\">")
            lines.append(f"    <h2>{location_name}</h2>")
            lines.append("    <ul class=\"reports\">")
            
            # Find all forecast files
            for md_file in sorted(loc_dir.glob("*.md")):
                lines.append(f"      <li><a href=\"by-location/{location_name}/{md_file.name}\">{md_file.name}</a></li>")
